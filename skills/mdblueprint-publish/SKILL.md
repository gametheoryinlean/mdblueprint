...
4. Run `uv run --extra browser python -m tools.knowledge.render_check /tmp/mdblueprint-site` for strict browser math verification when the browser runtime is installed.
5. Inspect the output under the chosen site directory for missing nodes, broken links, or graph errors.
6. Check every generated node page, not only the index:
...
7. Report the number of node pages checked and list any pages with LaTeX/rendering issues.
