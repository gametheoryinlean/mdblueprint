...
changes, duplicate or overlap triage, merge/split/generalization proposals,
proof-plan route separation, Lean/topic divergence, or impact analysis before
editing nodes.
...
6. Classify each proposed action:
...
7. Apply the generality gate when a proposal generalizes, splits, merges, or
   ...
   truth.
8. Produce a structured report. Use
   `references/refactor-report-schema.md` for durable reports.
9. If the user asks for actual edits, make focused changes only after the report
