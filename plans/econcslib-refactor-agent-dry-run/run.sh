...
INCLUDE_STAGED="${INCLUDE_STAGED:-0}"
...
- Review the admitted EconCSLib knowledge graph by default.
- Staged content is ${INCLUDE_STAGED}; use it only if the evidence packs explicitly include it.
...
6. For any new-node, split-node, missing-dependency, or generalization proposal, write request files under:
   - $RUN_ROOT/requests/
7. For concrete mechanical actions, write an explicit dry-run plan under:
   - $RUN_ROOT/dry-runs/refactor-plan.yml
   Use only operations supported by dry-run-plan-schema.md.
8. Run these validators before finishing:
   ...
   - uv --cache-dir /tmp/uv-cache run python -m tools.knowledge.refactor_dry_run "$KB_ROOT" "$RUN_ROOT/dry-runs/refactor-plan.yml" --json
9. Save the report here:
   - $RUN_ROOT/reports/econcslib-graph-refactor-report.md
10. Save dry-run JSON here:
   - $RUN_ROOT/dry-runs/refactor-dry-run.json
11. Save a short human summary here:
   - $RUN_ROOT/SUMMARY.md
