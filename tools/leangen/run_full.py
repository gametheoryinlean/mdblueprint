...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.lean_first:
        raise SystemExit('lean-first extraction is legacy-only and lives on the stale branch')
    ...
    for idx, lean_file in enumerate(lean_files, start=1):
        ...
        if theorem_path.exists() and theorem_path.stat().st_size > 0:
            theorem_payload = json.loads(theorem_path.read_text(encoding='utf-8'))
            all_theorems.extend(theorem_payload)
